...
/**
 * A pragmatic, deterministic floor extractor. It does NOT use a neural model;
 * it runs a transparent CV pass with sharp:
 *   grayscale → downscale → background estimate from the border →
 *   content mask (boundary bbox) → dark connected components (obstacle hints).
 *
 * Output is intentionally a rough proposal — the UI requires the user to
 * review/adjust and to calibrate real-world scale. A stronger extractor can be
 * dropped in behind the FloorExtractor interface later.
 */
export class HeuristicFloorExtractor implements FloorExtractor {
  ...
  async extract(image: Buffer): Promise<FloorProposal> {
    ...
    const border: number[] = [];
    for (let x = 0; x < w; x++) {
      border.push(lum(x, 0), lum(x, h - 1));
    }
    for (let y = 0; y < h; y++) {
      border.push(lum(0, y), lum(w - 1, y));
    }
    const bg = median(border);

    // Content mask: pixels that differ meaningfully from the background.
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        // notably darker than background → potential wall/obstacle
    ...
    if (contentCount > w * h * 0.01 && maxX > minX && maxY > minY) {
      ...
      // Could not find clear content — propose the full image as the floor.
      boundary = rectPoly(0, 0, fullW, fullH);
      confidence = 0.2;
    }

    // Obstacle hints: largest dark connected components inside the boundary.
  ...
  for (let i = 0; i < dark.length; i++) {
    ...
    while (stack.length) {
      ...
      for (const n of neighbors) {
        ...
        // avoid wrapping across rows for horizontal neighbors
    ...
    const bw = maxX - minX;
    const bh = maxY - minY;
    // keep reasonably sized, roughly blob-like regions (skip thin lines/borders)
